                                    ...
                                    sidebarItems.map((singleItem, index) => {
                                        ...
                                        if (singleItem?.items) {
                                            ...
                                            return <div className="flex flex-col justify-start items-center   px-6 border-b border-gray-600 w-full  ">
                                                <button onClick={() => openMenu(index)} className="focus:outline-none focus:text-indigo-400 text-left  text-white flex justify-between items-center w-full py-5 space-x-14  ">
                                                    <p className="text-left text-sm leading-5 uppercase">{singleItem.label}</p>
                                                    <svg id="icon1" className={`${state.menu != index ? "rotate-180" : "rotate-0"} "transform"`} width={24} height={24} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M18 15L12 9L6 15" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                                    </svg>
                                                </button>
                                                <div id="menu1" className={`${state.menu != index ? "hidden" : ""} flex justify-start flex-col w-full md:w-auto items-start pb-1 `}>
                                                    {singleItem.items.map(subItem => {
                                                        if (!permissions.includes(subItem.permissions) && !isAdmin) return;
                                                        return <button onClick={() => navigate(subItem.path)} className="flex justify-start items-center space-x-6 hover:text-white focus:bg-gray-700 focus:text-white hover:bg-gray-700 text-gray-400 rounded px-3 py-2  w-full md:w-52">
                                                            <div className="w-4">
                                                                {subItem.icon}
                                                            </div>
                                                            <p className="text-left text-base leading-4" style={{ marginLeft: "15px" }}>{subItem.label}</p>
                                                        </button>
                                                    })
                                                    }
                                                </div>
                    ...
                    <ul className="navbar-nav">
                        {
                            sidebarItems.map(item => {
                                ...
                                // }
                                return <li className="nav-item">
                                    <Link className={active == item?.path?.split("/")[3] || item?.matchings?.includes(active) ? "nav-link active" : "nav-link"} to={item?.path}>
                                        <div className="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                                            {item.icon}
                                        </div>
